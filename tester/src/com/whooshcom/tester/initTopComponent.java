...
public final class initTopComponent extends TopComponent {
    ...
    private void initializeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_initializeActionPerformed
        Document doc;
        int baseMAC=0x0A5000;
        String sMAC="40-D8-55-";
        
        // Calculate mac addresses
        int serialNumber = (Integer) serNum.getValue();
        int maintMac = baseMAC + ((serialNumber - 1001) * 2);
        int gigeMac = baseMAC + ((serialNumber - 1001) * 2) + 1;
        int m3 = (maintMac >> 16) & 0x00ff;
        int m2 = (maintMac >> 8) & 0x00ff;
        int m1 = maintMac & 0x00ff;
        int e3 = (gigeMac >> 16) & 0x00ff;
        int e2 = (gigeMac >> 8) & 0x00ff;
        int e1 = gigeMac & 0x00ff;
        ...
        lOper.setText("Operator=" + operator.getText());

        // try to connect to device    
        try {
            doc = Jsoup.connect("http://ADMIN:admin@192.168.34.123").timeout(3000).get();
        } catch (IOException ex) {
            System.out.println(" No device at http://ADMIN:admin@192.168.34.123");
        ...
        try {
            ...
            System.out.println("Error reading back box status");
            //Exceptions.printStackTrace(ex);
        }
